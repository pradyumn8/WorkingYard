...
const TaskTimer = ({ task }) => {
  ...
  // Base tracked seconds from DB
  const baseTracked = task.trackedSeconds ?? 0;

  // If timer running, add diff from timerStartedAt
  const initialSeconds = useMemo(() => {
    if (task.timerRunning && task.timerStartedAt) {
      const diff = Math.floor((Date.now() - new Date(task.timerStartedAt).getTime()) / 1000);
      return baseTracked + Math.max(0, diff);
    }
    return baseTracked;
  }, [baseTracked, task.timerRunning, task.timerStartedAt]);

  const [seconds, setSeconds] = useState(initialSeconds);
  const [isRunning, setIsRunning] = useState(Boolean(task.timerRunning));

  // Keep local state in sync if redux task changes (important)
  useEffect(() => {
    setIsRunning(Boolean(task.timerRunning));
    setSeconds(initialSeconds);
  }, [task.timerRunning, initialSeconds]);
  ...
  const toggleTimer = async (e) => {
    ...
    try {
      ...
      // ✅ update local UI state (server truth)
      setIsRunning(Boolean(data.task.timerRunning));

      // When pause happens server sends updated trackedSeconds
      const newTracked = data.task.trackedSeconds ?? 0;
      if (!data.task.timerRunning) setSeconds(newTracked);
  ...
  const s = seconds % 60;
  const label = [h, m, s].map((v) => String(v).padStart(2, "0")).join(":");

  return (
    <button
      onClick={toggleTimer}
      className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800"
    >
      <span>{label}</span>
      <span className="text-[10px] uppercase">{isRunning ? "Pause" : "Start"}</span>
    </button>
  );
};
...
const ProjectTasks = ({ tasks }) => {
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    try {
      ...
      let updatedTask = structuredClone(
        ...
        {["status", "type", "priority", "assignee"].map((name) => {
          ...
          return (
            <select
              ...
              className="border not-dark:bg-white border-zinc-300 dark:bg-zinc-800 dark:border-zinc-800 outline-none px-3 py-1 rounded text-sm text-zinc-900 dark:text-zinc-200"
            >
            ...
            <table className="min-w-full text-sm text-left not-dark:bg-white text-zinc-900 dark:text-zinc-300">
              <thead className="text-xs uppercase dark:bg-zinc-800/70 text-zinc-500 dark:text-zinc-400">
                <tr>
                  ...
                  <th className="px-4 pl-0 py-3">Task</th>
                  ...
                  <th className="px-4 py-3">Time Tracking</th>
                </tr>
              </thead>
                      ...
                      <tr key={task.id} onClick={() => navigate(`/taskDetails?projectId=${task.projectId}&taskId=${task.id}`)} className=" border-t border-zinc-300 dark:border-zinc-800 group hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-all cursor-pointer" >
                        ...
                        <td className="px-4 py-2">
                          {Array.isArray(task.watchers) &&
                            ...
                            <span className="text-xs text-zinc-700 dark:text-zinc-300">
                              {task.watchers.length} watcher
                              {task.watchers.length > 1 ? "s" : ""}
                            </span>
                          ) : (
                        ...
                        >
                          <select
                            ...
                            value={task.status}
                            className="group-hover:ring ring-zinc-100 outline-none px-2 pr-4 py-1 rounded text-sm text-zinc-900 dark:bg-zinc-900 dark:text-zinc-200 cursor-pointer bg-transparent"
                          >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs text-zinc-600 dark:text-zinc-400 flex items-center gap-2">
                      ...
                      <span
                        className={`${color || "text-zinc-500"} uppercase`}
                      >
                        {task.type}
                      </span>
                    </div>
                    ...
                    <div>
                      <span
                        className={`text-xs px-2 py-1 rounded ${background || ""
                          } ${prioritycolor || ""}`}
                      >
                        {task.priority}
                      </span>
